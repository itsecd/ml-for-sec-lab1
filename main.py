...
def classify(model_file_name, data_file_name):
    ...
    logger.debug(f"Чтение данных из файла {data_file_path}...")
    data = pd.read_csv(data_file_name, error_bad_lines="skip")
    logger.debug("Данные считаны успешно")

    logger.debug(f"Загрузка модели из файла {model_file_name}...")
    model = load(model_file_name)
    logger.debug("Модель успешно загружена")

    logger.debug(f"Прогнозирование...")
    prediction_array = np.array(model.predict(data))
    logger.debug("Прогнозирование завершено")
